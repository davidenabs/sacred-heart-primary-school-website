                            ...
                            <div class="card-header d-flex justify-content-between">
                                ...
                                <div>
                                    @if (!$post->is_published)
                                        <a href="{{ route('admin.posts.publish', $post->slug) }}"
                                            class="btn text-light btn-sm btn-info">Publish</a>
                                    @endif
                                    @if ($post->is_draft)
                                        <a href="{{ route('admin.posts.undraft', $post->slug) }}"
                        ...
                        <article class="article round">
                            ...
                            <div class="article-details">
                                ...
                                <div class="article-cta">
                                    <a href="{{ route('admin.posts.comments', $post->slug) }}" class="btn btn-primary">See
                                        comments</a>
                                </div>
                            </div>
                        </article>
                    ...
                    <div class="col-12 col-sm-12 col-lg-5">
                        <div class="card profile-widget">
                            <div class="profile-widget-header">
                                <img alt="image" src="{{  $post->author->writerInfo->profile ? asset($post->author->writerInfo->profile) : asset('frontend/assets/img/profile.jpg')  }}"
                                ...
                                <div class="profile-widget-items">
                                    <div class="profile-widget-item">
                                        ...
                                        <div class="profile-widget-item-value">
                                            @php
                                                $userPosts = App\Models\User::withTrashed()
                                                    ->whereId($post->author->id)
                                                    ->first();
                                                echo $userPosts->posts->count();
                                            @endphp
                                        </div>
                                    </div>
                                    <div class="profile-widget-item">
                                        <div class="profile-widget-item-label">Post Views</div>
                                        <div class="profile-widget-item-value">
                                            {{-- {{
                                            App\Models\User::withTrashed()->whereId($post->author->id)->get()->posts()->count()
                                            }} --}}
                                            0
                                        </div>
                                    </div>
                            ...
                            <div class="profile-widget-description pb-0">
                                <div class="profile-widget-name">{{ $post->author->name }} <div
                                        class="text-muted d-inline font-weight-normal">
                                        <div class="slash"></div> {{ $post->author->role }}
                                    </div>
                                </div>
                                <p>{{ $post->author->bio }}</p>
                            </div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            var route = @json(route('admin.analytics.posts', $post->slug));
                ...
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            ...
                            location.reload("{{ route('admin.posts.index') }}")
                        } else {
