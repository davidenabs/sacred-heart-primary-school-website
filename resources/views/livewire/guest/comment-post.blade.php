<div class="mb-5" id="comment">
    ...
    <div id="comment-{{ $key }}" class="media mb-4 revealfade-right">
        ...
        <div class="media-body">
            <h6>{{ $comment->name }} <small><i>{{ date('M d, Y \a\t h:ia', strtotime($comment->created_at)) }}</i></small></h6>
            <p>{{ $comment->content }}</p>
            <div class="d-flex justify-content-between">
                <a href="#comment-reply" class="btn btn-sm btn-light" wire:click="repyId('{{ $comment->id }}', '{{ $comment->email }}')">Reply</a>
                @if ($is_admin)
    ...
    <div class="{{ $is_admin ? 'rounded-lg border' : 'bg-light' }} p-5 revealfade-bottom" id="comment-reply">
        ...
        <form wire:submit.prevent="storeComment()" method="POST">
            ...
            <div class="form-group mb-0">
                {{-- <input type="submit" wire:target="storeComment" wire:loading.attr="disabled" value="Leave Comment" class=""> --}}
                <button
                    type="submit"
                    wire:target="storeComment"
                    wire:loading.attr="disabled"
                    class="btn btn-sm btn-round {{ $is_admin ? 'btn-primary' : 'shs-bg-primary-color' }} text-white"
                    >
                    <span wire:target="storeComment"
                        wire:loading.remove>Comment</span>
                    <span wire:target="storeComment"
                        wire:loading>Commenting...</span>
                </button>
            </div>
        </form>
    </div>
